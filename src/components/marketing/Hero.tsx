...
function AnimatedCounter({ target, prefix = "" }: { target: number; prefix?: string }) {
  ...
  const rounded = useTransform(count, (v) => `${prefix}${Math.round(v).toLocaleString("en-GB")}`);
...
function DrawNumbers() {
  ...
  return (
    <div className="relative">
      <div className="rounded-2xl border border-border/50 bg-background/60 p-8 backdrop-blur-xl">
        ...
        <div className="flex items-center justify-center gap-3">
          {numbers.map((n, i) => (
            <m.div
              ...
              className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-accent bg-accent/10 sm:h-16 sm:w-16"
            >
        ...
        <div className="grid items-center gap-16 lg:grid-cols-2 lg:gap-20">
          <div>
            ...
            >
              <Link
                href="/signup"
                className="inline-flex h-12 items-center gap-2 rounded-xl bg-accent px-6 text-[15px] font-semibold text-white shadow-lg shadow-accent/25 transition-all hover:bg-accent/90 hover:shadow-xl"
              >
                Start playing
              ...
              <Link
                href="/draws"
                className="inline-flex h-12 items-center rounded-xl border border-border bg-background px-6 text-[15px] font-medium text-foreground transition-colors hover:bg-muted"
              >
                See last draw
              </Link>
            </m.div>
